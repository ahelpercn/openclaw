...
export function renderDebug(props: DebugProps) {
  ...
  const securityLabel =
    critical > 0
      ? t("debug.snapshots.criticalIssues", { count: String(critical) })
      : warn > 0
        ? t("debug.snapshots.warnings", { count: String(warn) })
        : t("debug.snapshots.noIssues");
      ...
      <div class="card">
        <div class="row" style="justify-content: space-between;">
          ...
          <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
            ${props.loading ? t("common2.refreshing") : t("common2.refresh")}
          </button>
        </div>
            ...
            ${
              securitySummary
                ? html`<div class="callout ${securityTone}" style="margin-top: 8px;">
                  ${t("debug.snapshots.securityAudit")} ${securityLabel}${info > 0 ? ` · ${info} info` : ""}. Run
                  <span class="mono">${t("debug.snapshots.runAudit")}</span> ${t("debug.snapshots.auditHint")}
                </div>`
                : nothing
            }
