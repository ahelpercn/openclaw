...
export function renderCron(props: CronProps) {
  ...
  const selectedRunTitle = selectedJob?.name ?? props.runsJobId ?? t("cron.runHistory.selectJob");
  ...
  return html`
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          <div class="stat">
            ...
            <div class="stat-value">
              ${props.status ? (props.status.enabled ? t("channels.generic.yes") : t("channels.generic.no")) : t("channels.generic.na")}
            </div>
          </div>
          <div class="stat">
            <div class="stat-label">${t("cron.scheduler.jobs")}</div>
            <div class="stat-value">${props.status?.jobs ?? t("channels.generic.na")}</div>
          </div>
          <div class="stat">
      ...
      <div class="card">
        ...
        <div class="form-grid" style="margin-top: 16px;">
          ...
          <label class="field">
            ...
            >
              ...
              <option value="cron">${t("cron.newJob.cron")}</option>
            </select>
          </label>
        </div>
        ...
        <div class="form-grid" style="margin-top: 12px;">
          <label class="field">
            ...
            >
              <option value="main">${t("cron.newJob.main")}</option>
              <option value="isolated">${t("cron.newJob.isolated")}</option>
            </select>
          </label>
          <label class="field">
        ...
        <div class="form-grid" style="margin-top: 12px;">
          <label class="field">
            <span>${t("cron.newJob.delivery")}</span>
            <select
            ...
            >
              ${
                supportsAnnounce
                  ? html`
                      <option value="announce">${t("cron.newJob.announceSummary")}</option>
                    `
                  : nothing
              }
              <option value="webhook">${t("cron.newJob.webhook")}</option>
              <option value="none">${t("cron.newJob.none")}</option>
            </select>
          </label>
          ${
          ...
          ${
            selectedDeliveryMode !== "none"
              ? html`
                  <label class="field">
                    <span>${selectedDeliveryMode === "webhook" ? t("cron.newJob.webhookUrl") : t("cron.newJob.channel")}</span>
                    ${
                      selectedDeliveryMode === "webhook"
                        ? html`
                            <input
                              ...
                              placeholder=${t("cron.newJob.webhookPlaceholder")}
                            />
                          `
                        : html`
                            <select
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="card-sub">${t("cron.runHistory.subtitle", { title: selectedRunTitle })}</div>
      ${
        props.runsJobId == null
          ? html`
              <div class="muted" style="margin-top: 12px">${t("cron.runHistory.selectHint")}</div>
            `
...
function renderScheduleFields(props: CronProps) {
  ...
  if (form.scheduleKind === "every") {
    return html`
      <div class="form-grid" style="margin-top: 12px;">
        <label class="field">
          <span>${t("cron.newJob.everyAmount")}</span>
          <input
        ...
        <label class="field">
          <span>${t("cron.newJob.everyUnit")}</span>
          <select
          ...
          >
            <option value="minutes">${t("cron.newJob.minutes")}</option>
            <option value="hours">${t("cron.newJob.hours")}</option>
            <option value="days">${t("cron.newJob.days")}</option>
          </select>
        </label>
      </div>
    `;
  ...
  return html`
    <div class=${itemClass} @click=${() => props.onLoadRuns(job.id)}>
      <div class="list-main">
        ...
        ${job.agentId ? html`<div class="muted cron-job-agent">${t("cron.jobs.agent")} ${job.agentId}</div>` : nothing}
      </div>
      <div class="list-meta">
        ${renderJobState(job)}
      </div>
...
function renderJobPayload(job: CronJob) {
  if (job.payload.kind === "systemEvent") {
    return html`<div class="cron-job-detail">
      <span class="cron-job-detail-label">${t("cron.jobs.system")}</span>
  ...
  return html`
    <div class="cron-job-detail">
      <span class="cron-job-detail-label">${t("cron.jobs.prompt")}</span>
      <span class="muted cron-job-detail-value">${job.payload.message}</span>
    </div>
    ${
      delivery
        ? html`<div class="cron-job-detail">
            <span class="cron-job-detail-label">${t("cron.jobs.delivery")}</span>
  ...
  return html`
    <div class="list-item">
      ...
      <div class="list-meta">
        ...
        ${
          chatUrl
            ? html`<div><a class="session-link" href=${chatUrl}>${t("cron.runHistory.openChat")}</a></div>`
            : nothing
        }
