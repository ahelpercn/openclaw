...
export async function monitorDiscordProvider(opts: MonitorDiscordOpts = {}) {
  ...
  client.on(Events.MessageCreate, async (message) => {
    try {
      ...
      const guildSlug =
        guildInfo?.slug || (message.guild?.name ? normalizeDiscordSlug(message.guild.name) : "");
      ...
      if (isDirectMessage && Array.isArray(allowFrom) && allowFrom.length > 0) {
        const allowList = normalizeDiscordAllowList(allowFrom, ["discord:", "user:"]);
        const permitted =
          allowList &&
      ...
      const fromLabel = isDirectMessage ? buildDirectLabel(message) : buildGuildLabel(message);
      ...
      const ctxPayload = {
        Body: combinedBody,
        From: isDirectMessage ? `discord:${message.author.id}` : `group:${message.channelId}`,
        To: isDirectMessage ? `user:${message.author.id}` : `channel:${message.channelId}`,
      ...
      const sendBlockReply = (payload: ReplyPayload) => {
        if (!payload?.text && !payload?.mediaUrl && !(payload?.mediaUrls?.length ?? 0)) {
          return;
        }
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      const channelType = interaction.channel?.type as ChannelType;
      const isGroupDm = channelType === ChannelType.GroupDM;
  ...
  if (channelEntries && Object.keys(channelEntries).length > 0) {
    const entry =
      ...
      (channelSlug
        ...
        : undefined) ??
      (channelName ? channelEntries[normalizeDiscordSlug(channelName)] : undefined);
    if (!entry) {
...
function resolveSlashPrompt(options: readonly CommandInteractionOption[]): string | undefined {
...
function findFirstStringOption(options: readonly CommandInteractionOption[]): string | undefined {
